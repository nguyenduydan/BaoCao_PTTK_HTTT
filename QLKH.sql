...
/* Created on:     17/10/2023 10:19:28 CH                       */
/*==============================================================*/
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('INVOICE_PAYMENT') and o.name = 'FK_INVOICE__DETAILS_ORDERS_D')
alter table INVOICE_PAYMENT
   drop constraint FK_INVOICE__DETAILS_ORDERS_D
go
...
if exists (select 1
           ...
           where  id    = object_id('INVOICE_PAYMENT')
            ...
            and   indid > 0
            and   indid < 255)
   drop index INVOICE_PAYMENT.DETAILS_FK
go
...
create table INVENTORY_LEVEL (
   ...
   LEVE_ID              int                  not null,
   NAME                 text                 null,
   AMOUNT               numeric(5)           null,
   UPDATE_AT            datetime             null,
   EXPIRATION_DATE      datetime             null,
   constraint PK_INVENTORY_LEVEL primary key nonclustered (PRODUCT_ID, LEVE_ID)
)
go
...
/* Index: DETAILS_FK                                            */
/*==============================================================*/
create index DETAILS_FK on INVOICE_PAYMENT (
PAYMENTID ASC,
SUPPLIERID ASC
)
go
...
create table ITEMS (
   ...
   constraint PK_ITEMS primary key (SUPLLIER_ID, PRODUCTS_ID)
)
go
...
create table ORDERS_DETAIL (
   ...
   AMOUNT               numeric(5)           null,
   DATE                 datetime             null,
   constraint PK_ORDERS_DETAIL primary key nonclustered (PAYMENTID, SUPPLIERID)
)
go
...
create table SEND (
   ...
   SUPPLIERID           int                  not null,
   SUPLLIER_ID          int                  not null,
   constraint PK_SEND primary key (PAYMENTID, SUPPLIERID, SUPLLIER_ID)
)
go
...
create table STORAGE (
   ...
   PRODUCT_ID           int                  not null,
   LEVE_ID              int                  not null,
   constraint PK_STORAGE primary key (PRODUCTS_ID, PRODUCT_ID, LEVE_ID)
)
go
...
create table SUPPLIER (
   ...
   ADDRESS              text                 null,
   constraint PK_SUPPLIER primary key nonclustered (SUPLLIER_ID)
)
go

alter table INVOICE_PAYMENT
   add constraint FK_INVOICE__BILLS_SUPPLIER foreign key (SUPLLIER_ID)
      references SUPPLIER (SUPLLIER_ID)
go

alter table INVOICE_PAYMENT
   add constraint FK_INVOICE__DETAILS_ORDERS_D foreign key (PAYMENTID, SUPPLIERID)
      references ORDERS_DETAIL (PAYMENTID, SUPPLIERID)
go

alter table ITEMS
   add constraint FK_ITEMS_ITEMS_SUPPLIER foreign key (SUPLLIER_ID)
      references SUPPLIER (SUPLLIER_ID)
go
...
alter table SEND
   add constraint FK_SEND_SEND_ORDERS_D foreign key (PAYMENTID, SUPPLIERID)
      references ORDERS_DETAIL (PAYMENTID, SUPPLIERID)
go

alter table SEND
   add constraint FK_SEND_SEND2_SUPPLIER foreign key (SUPLLIER_ID)
      references SUPPLIER (SUPLLIER_ID)
go
...
alter table STORAGE
   add constraint FK_STORAGE_STORAGE2_INVENTOR foreign key (PRODUCT_ID, LEVE_ID)
      references INVENTORY_LEVEL (PRODUCT_ID, LEVE_ID)
go
